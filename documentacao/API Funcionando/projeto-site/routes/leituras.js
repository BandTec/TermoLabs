...
router.get('/ultimas/:fkSensor', function(req, res, next) {
	...
	const instrucaoSql = `select top ${limite_linhas} 
							...
							from leitura 
							inner join sensorTemperatura on
							fkSensor = idSensor
...
router.get('/tempo-real/:fkSensor', function(req, res, next) {
	...
	let instrucaoSql = `select top 1 temperatura, FORMAT(momento,'HH:mm:ss') as momento from leitura inner join sensorTemperatura on fkSensor = idSensor where ${fkSensor} = idSensor order by id desc`;
